...
CAMPO_DATA = 'data_ligacao_ini'
TABELA_ALVO = 'tb_im_hw'
...
def batch(dia)-> dict:
    ...
    ANOMES = int((datetime.now() - timedelta(days=dia)).strftime('%Y%m'))
    ...
    PRESTMT = f"DELETE FROM {TABELA_DESTINO} FORCE INDEX (AnoMes) WHERE ano_mes = {ANOMES} AND {CAMPO_DATA} = '{ETL_DATA}'"
    WHERE = f"FORCE INDEX (AnoMes) WHERE ano_mes = {ANOMES} AND {CAMPO_DATA} = '{ETL_DATA}'"
    UPDATE_STMT = f"FORCE INDEX (AnoMes) WHERE ano_mes = {ANOMES} AND data_atualiza = '{ETL_DATA}'"
    COLUNAS = ['*']
...
if __name__ == '__main__':
    nrt()
    # for dia in reversed(range(0,33)):
    #     ETL_DATA = (datetime.now() - timedelta(days=dia)).strftime('%Y-%m-%d')
    #     print(ETL_DATA)
    #     batch(dia)
