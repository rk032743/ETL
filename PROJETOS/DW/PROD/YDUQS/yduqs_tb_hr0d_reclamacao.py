...
TABELA_ALVO = 'tb_hr0d_reclamacao'
...
def reproc():
    ...
    global parametros
    dt_ini = '2023-01-01'
    datas = (pd.date_range(dt_ini,(datetime.now() - timedelta(days=1)),freq='d'))
    parametros = dict()
    parametros['PRESTMT'] = f"DELETE FROM {TABELA_DESTINO} WHERE {CAMPO_DATA_2} >= '{dt_ini}'"
    parametros['BANCO_DESTINO'] = BANCO_DESTINO
    prep()
    print('AMBIENTE PRONTO!')
    print(parametros['PRESTMT'])
    print(parametros['BANCO_DESTINO'])
