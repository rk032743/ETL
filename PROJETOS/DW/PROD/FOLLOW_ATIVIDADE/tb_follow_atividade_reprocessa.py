...
BANCO_ORIGEM = 'bd_bi_orizon'
BANCO_DESTINO = 'cxg_db_orizon'
CAMPO_DATA = 'data_cri'
...
def maintance() -> str:
    ...
    TB_CURRENT = 'tb_follow_atividade'
    TB_OLD = f'tb_follow_atividade_{ANOMES}'
...
def batch(dia, tb)-> dict:
    ...
    CREATE = f'CREATE TABLE IF NOT EXISTS {tb} LIKE tb_follow_atividade;'
    engine_destination = db.conn_engine(1, BANCO_DESTINO)
    with engine_destination.connect() as conn:
        cmd1 = CREATE
        conn.execute(text(cmd1))
...
if __name__ == '__main__':
    start_process = datetime.now()
    for dia in range(19,170):
        ETL_DATA = (datetime.now() - timedelta(days=dia)).strftime('%Y-%m-%d')
        print(ETL_DATA)
        tb_data = pd.to_datetime(ETL_DATA).strftime('%y%m')
        tabela = f'tb_follow_atividade_{tb_data}'
        print(tabela)
        batch(dia,tabela)
    end_process = datetime.now()
    duration = end_process - start_process
