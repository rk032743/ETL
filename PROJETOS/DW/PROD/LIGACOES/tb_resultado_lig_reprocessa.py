...
def maintance() -> str:
    ...
    TB_CURRENT = 'tb_a_resultado_lig'
    TB_OLD = f'tb_a_resultado_lig{ANOMES}'
    CREATE = f'CREATE TABLE IF NOT EXISTS {TB_CURRENT} LIKE {TB_OLD};'
...
def d1(dia, tb) -> list:
    ...
    where = f"WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME = '{tb}' AND TABLE_SCHEMA = 'bd_bi_superdigital'"
    ...
    # CREATE = f'CREATE TABLE IF NOT EXISTS {tb} LIKE tb_a_resultado_lig;'
    # engine_destination = db.conn_engine(1, BANCO_DESTINO)
    # with engine_destination.connect() as conn:
    #     cmd1 = CREATE
    #     conn.execute(text(cmd1))
    ...
    # TABELA_DESTINO = tb
    TABELA_DESTINO = "tb_a_resultado_lig"
    # PRESTMT = f"DELETE FROM {TABELA_DESTINO} WHERE {CAMPO_DATA} = '{ETL_DATA}'"
    # PRESTMT = f"DELETE FROM {TABELA_DESTINO} WHERE {CAMPO_DATA} = '{ETL_DATA}' AND representante = 'SUPERDIGITAL'"
    PRESTMT = f'SELECT 1'
    p = prep(PRESTMT)
...
def extract()-> pd.DataFrame:
    ...
    SQL_ORIGEM = f'SELECT * FROM {tabela_origem} {where}'
    tabela_alvo = parametros['TABELA_ALVO']
    start_time = time.time()
    print(SQL_ORIGEM)
...
def transform(df)-> pd.DataFrame:
    ...
    columns_ex = []
    for x in COLUNAS_EXTRAS:
        columns_ex.append(x.strip("'"))
    ...
    new_cols = [*COLUNAS, *COLUNAS_EXTRAS]
    new_cols = [item.strip("'") for item in new_cols]
    ...
    for colname, coltype in df.dtypes.items():
        if coltype == 'timedelta64[ns]':
            ...
            df[colname] = df[colname].astype(str).apply(lambda x: x[7:])
    # REMOVE DADOS INVÁLIDOS.
    for colname in df.columns:
        # df[colname] = df[colname].astype(str).map(lambda x: x.replace('1111-11-11 00:00:00', 'NULL'))
        df[colname] = df[colname].astype(str).apply(lambda x: x.replace('1111-11-11 00:00:00', 'NULL'))
...
def load(df, tipo)->dict:
    ...
    if tipo == 0:
        ...
        db.loaddata(arquivo=parametros['ARQUIVO'], banco=parametros['BANCO_DESTINO'], tabela="dm_db_ligacoes.tb_a_resultado_lig2308", tipo=0, separador='§')
...
# for dia in range(1,156): 29/03
        dia = 7
        tabela = "tb_resultado_lig2308"
        d1(dia,tabela)
    # start_process = datetime.now()
    # for dia in reversed(range(1,7)):
    #     ETL_DATA = (datetime.now() - timedelta(days=dia)).strftime('%Y-%m-%d')
    #     print(ETL_DATA)
    #     tb_data = pd.to_datetime(ETL_DATA).strftime('%y%m')
    #     tabela = f'tb_a_resultado_lig{tb_data}'
    #     print(tabela)
    #     d1(dia,tabela)
    # end_process = datetime.now()
    # duration = end_process - start_process
    # print(f"TEMPO DECORRIDO: {duration}")