...
def conn_engine(id_db:int, banco:str):
    ...
    if id_db == 1:
        ...
        engine = create_engine(f"mssql+pyodbc://{dbconfig['username']}:{dbconfig['password']}@{dbconfig['host']}:{dbconfig['port']}/{dbconfig['db']}?driver={dbconfig['driver']}?CharacterSet={dbconfig['charset']}")
...
def loaddata(arquivo, banco, tabela, tipo, separador=';', *args):
