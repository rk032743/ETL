...
pprint(gera_datas())
# 1. Urls do projeto.
login_url = 'https://didactik.grupokonecta.com/login/index.php'
...
# 3. Recebe o HTML e localiza o token que foi gerado.   
        ...
        for tag in soup.find_all("input", type="hidden", attrs={"name": "logintoken"}):
            token = tag["value"]
            logging.info(f"Token: {token}")

# 4. Efetua o login com as credenciais e o token.
        r = session.post(login_url, data=payload(token))
        soup = BeautifulSoup(r.content, 'html.parser')
